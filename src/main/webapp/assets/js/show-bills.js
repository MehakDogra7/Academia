...
async function start() {
    ...
    document.getElementById("welcomeMessage").innerHTML = "Welcome " + firstName + " To Fee Payment Portal of IIIT Bangalore";
    document.getElementById("rollNumber").innerHTML = "Roll Number::" + userName;
    ...
    if (billsList.length === 0) {
        ...
        document.getElementById('heading').innerHTML = "Your Pending Bills are listed below:";
    ...
    if (billsList.length === 0) {
        ...
        for (let i = 0; i < billsList.length; i++) {
            ...
            if (remain < 3) {
                if (remain === 0) {
                    temp += '<a>' + billsList[i].description + ' Last day to pay...' + '</a>';
                } else if (remain > 0) {
                    temp += '<a>' + billsList[i].description + ' Deadline overs in ' + remain + ' Days' + '</a>';
                } else {
                    temp += '<a>' + billsList[i].description + ' Bro You are late. Submit today + '+ '</a>';
                }
                counter++;
            }
        }
